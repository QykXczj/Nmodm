    ...
    def run(self):
        ...
        try:
            ...
            if not source.exists():
                raise Exception(f"源路径不存在: {self.source_path}")

            self.progress_updated.emit(30, "正在压缩存档文件...")
                ...
                if source.is_file():
                    ...
                    for file_path in source.rglob('*'):
                        if file_path.is_file():
                            ...
                            self.progress_updated.emit(progress, f"正在压缩文件 {processed}/{total_files}")
            ...
            self.backup_finished.emit(f"备份创建成功！\n文件: {self.backup_name}\n大小: {size_mb:.1f}MB\n位置: {backup_file}")

        except Exception as e:
            self.error_occurred.emit(f"备份失败: {str(e)}")
...
class SaveRestoreWorker(QThread):
    ...
    def run(self):
        """执行恢复"""
        try:
            self.progress_updated.emit(10, "正在准备恢复...")

            # 检查备份文件是否存在
            backup_path = Path(self.backup_file)
            if not backup_path.exists():
                raise Exception(f"备份文件不存在: {self.backup_file}")

            # 检查目标路径
            target = Path(self.target_path)

            self.progress_updated.emit(30, "正在解压备份文件...")
            ...
            with zipfile.ZipFile(backup_path, 'r') as zipf:
                ...
                for i, file_name in enumerate(file_list):
                    ...
                    self.progress_updated.emit(progress, f"正在恢复文件 {i+1}/{total_files}")

            self.progress_updated.emit(100, "恢复完成！")
            self.restore_finished.emit(f"存档恢复成功！\n已恢复到: {self.target_path}")
...
class MiscPage(BasePage):
    ...
    def create_save_converter_section(self):
        ...
        # 添加到水平布局，调整宽度比例
        # 存档转换区域：存档备份区域 = 1:2
        horizontal_layout.addWidget(converter_card, 1)
        horizontal_layout.addWidget(backup_card, 2)
    ...
    def create_converter_card(self):
        ...
        # 使用说明按钮
        self.usage_help_btn = QPushButton("📖 使用说明")
        self.usage_help_btn.setFixedSize(80, 28)
        ...
        warning_layout.addWidget(self.usage_help_btn)
...
AES-CBC解密算法处理存档文件替换其中的Steam ID并重新计算校验和确保文件完整性。"""
    ...
    def create_backup_card(self):
        ...
        """)
        layout.addWidget(title_label, 0, 0, 1, 2)  # 第0行，跨越2列
        ...
        self.backup_path_label = QLabel("请选择存档文件夹路径...")
        ...
        select_path_btn.setFixedSize(32, 32)
        select_path_btn.setToolTip("选择存档文件夹")
        ...
        # 添加警告信息到标题右边
        warning_label = QLabel("⚠️ 恢复存档将覆盖当前存档！")
        warning_label.setStyleSheet("""
            QLabel {
                color: #f9e2af;
                font-size: 12px;
        ...
        """)
        title_row.addWidget(warning_label)
        ...
        self.auto_backup_checkbox = QCheckBox("启动时自动备份")
        ...
        """)
        settings_row.addWidget(keep_label)
    ...
    def create_backup(self):
        ...
        if not backup_path:
            dialog = NotificationDialog("警告", "请先选择存档文件夹路径", "warning", self)
            dialog.exec()
            return

        # 检查路径是否存在
        if not Path(backup_path).exists():
            dialog = NotificationDialog("错误", "选择的存档路径不存在", "error", self)
            dialog.exec()
            return

        # 禁用按钮
        self.create_backup_btn.setEnabled(False)
        self.create_backup_btn.setText("备份中...")
    ...
    def on_backup_progress(self, progress: int, status: str):
        ...
        self.create_backup_btn.setText(f"备份中... {progress}%")

    def on_backup_finished(self, message: str):
        """备份完成"""
        dialog = NotificationDialog("备份成功", message, "success", self)
        dialog.exec()
        self.refresh_backup_list()

    def on_backup_error(self, error_msg: str):
        """备份错误"""
        dialog = NotificationDialog("备份失败", error_msg, "error", self)
        dialog.exec()

    def on_backup_complete(self):
        """备份完成（无论成功失败）"""
        self.create_backup_btn.setEnabled(True)
        self.create_backup_btn.setText("创建备份")
    ...
    def restore_backup(self):
        ...
        if not current_item:
            dialog = NotificationDialog("提示", "请先选择要恢复的备份文件", "info", self)
            dialog.exec()
            return

        # 检查备份列表是否为空
        if self.backup_list.count() == 0 or current_item.text() in ["暂无备份文件", "备份列表功能正在开发中..."]:
            dialog = NotificationDialog("提示", "当前没有可用的备份文件", "info", self)
            dialog.exec()
            return
        ...
        if item_text in ["暂无备份文件", "备份列表功能正在开发中..."]:
            return
        ...
        if not backup_file.exists():
            dialog = NotificationDialog("错误", "备份文件不存在", "error", self)
            dialog.exec()
            return
        ...
        if confirm_dialog.exec() == ConfirmDialog.Accepted:
            ...
            self.restore_backup_btn.setText("恢复中...")
    ...
    def on_restore_progress(self, progress: int, status: str):
        """更新恢复进度"""
        self.restore_backup_btn.setText(f"恢复中... {progress}%")

    def on_restore_finished(self, message: str):
        """恢复完成"""
        dialog = NotificationDialog("恢复成功", message, "success", self)
        dialog.exec()

    def on_restore_error(self, error_msg: str):
        """恢复错误"""
        dialog = NotificationDialog("恢复失败", error_msg, "error", self)
        dialog.exec()

    def on_restore_complete(self):
        """恢复完成（无论成功失败）"""
        self.restore_backup_btn.setEnabled(True)
        self.restore_backup_btn.setText("恢复备份")
    ...
    def delete_backup(self):
        ...
        current_item = self.backup_list.currentItem()
        if not current_item:
            dialog = NotificationDialog("提示", "请先选择要删除的备份文件", "info", self)
            dialog.exec()
            return
        ...
        if confirm_dialog.exec() == ConfirmDialog.Accepted:
            try:
                backup_file.unlink()
                dialog = NotificationDialog("成功", "备份文件已删除", "success", self)
                dialog.exec()
                self.refresh_backup_list()
            except Exception as e:
                dialog = NotificationDialog("错误", f"删除失败: {str(e)}", "error", self)
                dialog.exec()
    ...
    def update_auto_backup_button_state(self):
        ...
        try:
            ...
            if not has_backup_path:
                ...
                self.auto_backup_checkbox.setToolTip("请先选择存档文件夹路径")
            else:
                self.auto_backup_checkbox.setToolTip("启动游戏时自动备份存档")
    ...
    def show_usage_help(self):
        ...
        dialog = QDialog(self)
        dialog.setWindowTitle("存档转换使用说明")
